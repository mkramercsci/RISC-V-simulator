...
using std::endl;
/**
 * Decodes binary into RV32i assembly language
  **/
class rv32i_decode : public hex
{
public:
	/**
	 * @param addr The location in simulated memory of the insn to be decoded
	 * @param insn The value in simulated memory to be decoded
	 * @return Decoded RISCV instructions, formatted as needed
	* **/
	static std::string decode(uint32_t addr, uint32_t insn);

protected:
	static constexpr int mnemonic_width = 8; ///< keep mneumonic column size uniform

	/**
	* @defgroup opcodes Opcodes
	* Identifiers for the type of instruction to be decoded
	*
	@{**/
	static constexpr uint32_t opcode_lui = 0b0110111; 	///< lui
	static constexpr uint32_t opcode_auipc = 0b0010111; 	///< auipc
	static constexpr uint32_t opcode_jal = 0b1101111; 	///< jal
	static constexpr uint32_t opcode_jalr = 0b1100111; 	///< jalr
	static constexpr uint32_t opcode_btype = 0b1100011; 	///< b type
	static constexpr uint32_t opcode_load_imm = 0b0000011; 	///< i type immediate
	static constexpr uint32_t opcode_stype = 0b0100011; 	///< s type
	static constexpr uint32_t opcode_alu_imm = 0b0010011; 	///< i type 
	static constexpr uint32_t opcode_rtype = 0b0110011; 	///< r type
	static constexpr uint32_t opcode_system = 0b1110011; 	///< system instructions
	/**@}*/

	/**
	* @defgroup funct3 Funct3
	* * Identifiers for the different instructions' funct3 values
	*
	@{**/
	...
	/**
	* @defgroup funct7 Funct7
	* Identifiers for the different instructions' funct7 values*
	* @{ **/	
	...
	* Identifiers for the different system instructions
	*
	*	@{ **/	
	...
	/**
	* @defgroup getX Get value
	* @param insn A binary instruction to have its value extracted from
	* @return Binary values of specific portions of the instruction
	* 	@{ **/
	...
	static constexpr uint32_t XLEN = 32; ///< The length in bits of an instruction

	/**
	* @defgroup render Render an instruction from binary to RISCV language
	* 
	* @param insn An instruction from simulated memory
	* @return Decoded, formatted machine language
	*	@{ **/
	static std::string render_illegal_insn(uint32_t insn); ///< An illegal instruction has been entered
	...
	///@param r A register to be formatted with a leading 'x'
	static std::string render_reg(int r);

	//@param m An instruction's mneumonic
